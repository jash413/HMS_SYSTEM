...
function PatientInvoices() {
  ...
  const fetchInvoices = async () => {
    try {
      const response = await axios.get("https://backendmedisys.webwisesolution.me/billing", {
        headers: {
    ...
    for (const invoice of invoices) {
      try {
        const [patientResponse, doctorResponse, wardResponse] =
          await Promise.all([
            axios.get(`https://backendmedisys.webwisesolution.me/api/patients/${invoice.patient}`, {
              headers: {
                authorization: `Bearer ${token}`,
              },
            }),
            axios.get(`https://backendmedisys.webwisesolution.me/doctors/${invoice.doctor}`, {
              headers: {
                authorization: `Bearer ${token}`,
              },
            }),
  ...
  const handleInvoiceDetails = async (invoice) => {
    try {
      const response = await axios.get(
        `https://backendmedisys.webwisesolution.me/billing/${invoice._id}`,
        {
          headers: {
            authorization: `Bearer ${token}`,
          },
  ...
  const handlePatientDetails = async (invoice) => {
    try {
      const response = await axios.get(
        `https://backendmedisys.webwisesolution.me/api/patients/${invoice.patient}`,
        {
          headers: {
            authorization: `Bearer ${token}`,
          },
  ...
  const handleHospitalDetails = async (invoice) => {
    try {
      const response = await axios.get(
        `https://backendmedisys.webwisesolution.me/api/hospital/${invoice.hospital_id}`,
        {
          headers: {
            authorization: `Bearer ${token}`,
          },
